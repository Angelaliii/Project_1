// booking.js - 全新改寫，移除嵌入CSS並確保點擊功能正常
document.addEventListener('DOMContentLoaded', function () {
  console.log('初始化教室預約系統');
  ...
  const bookingFormBox = document.getElementById('booking-form-container');
  ...
  function initialize() {
    ...
    if (timetable) {
      ...
      timetable.addEventListener('click', function (e) {
        ...
        if (isDragging || lastClickedCell === e.target.closest('.time-slot')) {
      ...
      // 滑鼠移動處理
      timetable.addEventListener('mouseover', function (e) {
      ...
      document.addEventListener('mouseup', function (e) {
        ...
        if (!dragStartSelected && !hasSelectedAfterStart) {
          dragRangeCells.forEach((cell) => {
            ...
            const isSelected =
              findSlotIndex(slot.classroomId, slot.hour) !== -1;
            if (!isSelected) {
              toggleSlot(
                cell,
                slot.classroomId,
